...
func (d DatasetResourceSrv) PrepareResource(runId string,  resource exports.GObject) (interface{},APIError){
	  ...
	  err := Request(configs.GetAppConfig().Resources.Dataset + "/api/apps/app_code_dataset/bind","POST",nil,req, &result)
	  if err != nil {
		return nil,err
  	  }
...
func (d DatasetResourceSrv) CompleteResource(runId string,resource exports.GObject,commitOrCancel bool) APIError {
		...
		err := Request(configs.GetAppConfig().Resources.Dataset + "/api/apps/app_code_dataset/unbind","POST",nil,req, nil)
